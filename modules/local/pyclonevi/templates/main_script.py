...
if __name__ == "__main__":
    create_pyclone_input(input_data="$rds_join", patient_id="$meta.patient",
                         output_data=opt["prefix"]+"_pyclone_input_all_samples.tsv")
    ...
    pyclonevi_write = "pyclone-vi write-results-file -i " + opt["prefix"] + "_all_fits.h5 -o " + \
                      opt["prefix"] + "_best_fit.txt"
    subprocess.run(pyclonevi_write, shell=True)

    # # Pyclone ctree
    pyclone_ctree(joint=opt["prefix"] + "_pyclone_input.tsv",
                  best_fit=opt["prefix"] + "_best_fit.txt",
                  ctree_input=opt["prefix"] + "_cluster_table.csv")

    # Version
    version = subprocess.check_output("pip show pyclone-vi | grep Version | awk '{print \$NF}'", shell=True).decode().split("\\n")[0]
    print(version)
